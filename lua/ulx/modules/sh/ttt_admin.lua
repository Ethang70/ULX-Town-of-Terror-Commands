...
function updateRoles()
	...
	table.insert(ulx.target_role,"survivalist")
	table.insert(ulx.target_role,"jester")
	table.insert(ulx.target_role,"phoenix")
...
function ulx.force( calling_ply, target_plys, target_role, should_silent )
	if not GetConVarString("gamemode") == "terrortown" then ULib.tsayError( calling_ply, gamemode_error, true ) else
		...
		if target_role ==  "survivalist" or target_role == "s" then role, role_grammar, role_string, role_credits = ROLE_SURVIVALIST, "a ", "survivalist", starting_credits end 
		if target_role ==  "jester"      or target_role == "j" then role, role_grammar, role_string, role_credits = ROLE_JESTER,      "a ", "jester", 0                          end
		if target_role ==  "phoenix" or target_role == "p" then role, role_grammar, role_string, role_credits = ROLE_RESURRECTOR,  "a ", "phoenix", 0                    end
